...
path_gcmc_structures = 'gcmc_selected_structures'
delete_unwanted_files_and_folders(path_gcmc_structures)
...
output_file = 'new_designed_structures/All_designed_structures.xlsx'
wb.save(output_file)


# 8. Original DNN model was used to predict the performance of all new Structures
...
# Loading data
data = pd.read_excel('new_designed_structures/All_designed_structures.xlsx', engine='openpyxl')
...
path_excel = 'new_designed_structures/All_designed_structures.xlsx'
...
Structurename_data = np.hstack((Structure_name.reshape(-1, 1), y_pred_1))

for i in range(len(Structurename_data)):
    Structurename = Structurename_data[i][0]
    Structure_TSN = Structurename_data[i][1]
    for j in range(row_count):
        if df.iloc[j, 0] == Structurename:
            ws.cell(row=j+2, column=tsn_col_index_0).value = Structure_TSN
            break

book.save(path_excel)


# 9. Choosing the Structures with top 10% TSN to be the training data
new_Structure_name_0 = os.listdir(path_new_structure_1)
new_Structure_name = []
for j0 in new_Structure_name_0:
    if '.cif' not in j0:
        pass
    else:
        new_Structure_name.append(j0)
        
order = np.argsort(y_pred.ravel())[::-1]
choose_number = round(train_test * len(order))
Structure_choosed = [Structure_name[i] for i in order[:choose_number]]
Structure_not_selected = [Structure_name for Structure_name in new_Structure_name if Structure_name not in Structure_choosed] # The rest of new structures

# Moving the chosen structures to a new file
for p in Structure_choosed:
    shutil.copy(path_new_structure_1 + '/' + p, path_gcmc_structures + '/' + p)
    
# Extracting the name of these structures which need to be calculated EQeq
written_content = os.listdir(path_gcmc_structures)
written_content_1 = []
header = ['Name']
...
output_file = 'LINUX/Structures_gcmc.csv'  
output_file1 = 'gcmc_selected_structures/Structures_gcmc.csv'
...
X_test, sub_name_node, sub_name_topo, linker_fp2, Structure_name, bbb = EM(path_gcmc_structures, path_mol)
...
for i in range(len(Structurename_data)):
    ...
    for j in range(row_count_df_output_file):
        if df_output_file.iloc[j, 0] == Structurename:
            ws.cell(row=j+2, column=tsn_col_index).value = Structure_TSN
            break

book1.save(output_file)


# Moving the not chosen Structures to a new file
for q in Structure_not_selected:
    shutil.copy(path_new_structure_1 + '/' + q, path_test_data + '/' + q)
    
X_test, sub_name_node, sub_name_topo, linker_fp2, Structure_name, bbb = EM(path_test_data, path_mol)
